<template>
    <el-container>
            <el-header class="animate__animated animate__fadeInDown">
                <el-menu default-active="label" class="el-menu-demo" mode="horizontal" @select="handleSelect" background-color="#545c64" text-color="#fff" active-text-color="#5fb878" router>
                    <el-menu-item index="/">
            ...
            <div class="container">
                <div class="row">
                    <div class="col-4" v-for="item in labellist" :key="item.labelD">
                        <div class="selfbox animate__animated animate__fadeInDownBig" @click="delink(item.labelID)">
                            <img src="../../static/img/timg.jpg" class="imgsize">
                            <div class="maincontent">
                                <el-button type="success" size="mini" round style="background-color:#2ec4b6; border-color:#2ec4b6">{{item.labelName}}</el-button><br/>
                            </div>
                        </div>
                    </div>                    
                </div>
...
export default {
    ...
    methods:{
        ...
        delink(k){
            this.$Axios.get('http://localhost:8014/labelDetail/?id='+k)
            .then(backdata => {
                this.articleData = backdata.data;
                this.$router.push({path:'/labeldetail',query:{data:JSON.stringify(this.articleData)}})
...
<style>
    ...
    .selfbox:hover{
        box-shadow:18px 24px 5px 3px#e4e4e4;
